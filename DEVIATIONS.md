...
### 2026-06-03: Acoustic Playback from a Stereo Pair Rather than a Single Speaker (Pre-data; recorded 2026-09-04)
**Phase:** Data Acquisition (Acoustic condition), apparatus setup
**Status:** Active deviation for the whole of main data collection. Recorded retrospectively; the configuration was in place before the first session.

* **Deviation:** §3 (Acoustic) specifies a single speaker positioned 1 m in front of the subject and oriented toward the head. Throughout data collection the pink-noise stimulus was played from a stereo pair of consumer speakers positioned in front of the participant, with the level set to the registered 65 ± 3 dB(A) target measured at head level. The manuscript describes the stereo configuration as used.
* **Motivation:** The stereo pair was the playback hardware in the recording room; the registered single-speaker geometry was not reproduced. The omission from this log until 2026-09-04 was an oversight, not a later change.
* **Impact:** None on any reported result. The stimulus, its spectrum, and its calibrated level at the head are as registered; only the source geometry differs. The acoustic condition produced no reliable EEG or classifier effect (Manuscript §IV-A, §V), so the source configuration does not bear on any conclusion. The phrase "single consumer speaker" in the impact note of the 2026-06-11 bit-depth entry describes the playback chain loosely and should be read as the stereo pair recorded here.
...
### 2026-09-01: Post-Hoc Marker-Alignment Diagnostic (Peer-review response)
**Phase:** Analysis (pre-proceedings revision)
**Status:** Post-hoc and exploratory. No registered or reported value is altered.

* **Deviation:** An additional diagnostic layer was added after peer review.
  Marker alignment (`analysis/loader.py`) fits acquisition time against packet
  arrival time by ordinary least squares, which assumes symmetric error.
  Bluetooth delay is one-sided -- a packet can arrive late, never early -- so
  where the delay distribution develops a heavy tail the fitted line is biased
  and every stimulus marker is displaced. Clock-fit residuals were computed for
  all 164 recordings of the 41 retained participants, each recording was re-scored under a delay-robust refit,
  and the reported conclusions were re-derived with alignment-affected subjects
  excluded. Code in `analysis/timing_diagnostic.py`, `run_timing_sweep.py` and
  `robustness_checks.py`; outputs in `data/derived/timing-diagnostic/` and
  `data/derived/robustness-checks/`, the latter holding
  `robustness_summary.json` as the canonical record of the figures below.

* **Motivation:** Reviewer 1 observed that reading the two near-chance EMI
  recordings as catastrophic failure caused by radiated RF energy coupling into
  the sensing chain "lacks supporting impedance or RF measurements." That is
  correct; no such measurements exist. The diagnostic tests an alternative
  explanation the recorded data can address.

* **Findings:** Residuals are approximately 6 ms in 153 of 164 recordings, 11
  exceed 30 ms and 3 exceed 90 ms. Condition medians are near-identical
  (6.01-6.49 ms) and flagged recordings occur in all four conditions, so the
  fault is not attributable to the EMI manipulation; the deliberate 200 Mbit/s
  2.4 GHz flood itself produces unremarkable timing. Host scheduling, headset
  battery, and position within the session were each tested and excluded. No
  samples were dropped in any affected recording, so the counter-based integrity
  check reported on 2026-06-11 does not detect this fault. The three recordings
  with the largest residuals are the three near-chance outliers in the
  manuscript, and all three recover under a delay-robust refit. The residual
  cuts used (30 ms, 90 ms) were chosen after the distribution was known; they
  are not pre-specified rules, and every check is therefore reported at both.

* **Impact:** None on reported results. The primary chewing-vs-control contrast
  is unchanged under exclusion at either cut (Cohen's d_z = -1.20 at N = 41,
  -1.14 at N = 38, -1.20 at N = 34, all p_Holm < 1e-7), as are the secondary
  N170 contrast (d = +0.49 to +0.47) and the chewing amplitude / rejection-rate
  correlation (r = 0.73 to 0.74). The EMI and acoustic contrasts, already
  non-significant, approach zero once affected recordings are removed,
  indicating that the between-subject spread previously described for EMI is
  contributed by those recordings rather than by the manipulation. A uniform
  correction is not adopted: Huber regression recovers two of the three severely
  misaligned recordings while leaving well-aligned ones untouched, RANSAC
  reproduces the least-squares fit exactly, and the more aggressive
  upper-envelope fit recovers all three but degrades well-aligned recordings,
  most visibly under chewing. That degradation tracks neither residual skewness
  nor surviving epoch count and is left unexplained. Since a uniform correction
  would move values in both directions and re-derive every reported number, the
  registered alignment is retained and this is reported as a diagnostic.
